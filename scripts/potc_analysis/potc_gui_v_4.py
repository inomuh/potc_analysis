    ...
    def click_start_analysis_button_func(self):
        try:
            ...
            if load_data_state and threshold_type_state:
                ...
                thread_potc_loaded = threading.Thread(target=potc_class_potc_loaded.calculate_main_potc_func)
                thread_potc_unloaded = threading.Thread(target=potc_class_potc_unloaded.calculate_main_potc_func)
                thread_reliability_loaded = threading.Thread(target=potc_class_reliability_loaded.calculate_main_potc_func)
                thread_reliability_unloaded = threading.Thread(target=potc_class_reliability_unloaded.calculate_main_potc_func)

                thread_potc_loaded.start()
                thread_potc_unloaded.start()
                thread_reliability_loaded.start()
                thread_reliability_unloaded.start()

                thread_potc_loaded.join()
                thread_potc_unloaded.join()
                thread_reliability_loaded.join()
                thread_reliability_unloaded.join()
                ...
                thread_loaded = threading.Thread(target=potc_class_loaded.calculate_main_potc_func)
                thread_unloaded = threading.Thread(target=potc_class_unloaded.calculate_main_potc_func)

                thread_loaded.start()
                thread_unloaded.start()

                thread_loaded.join()
                thread_unloaded.join()
                ...
                thread_potc = threading.Thread(target=potc_class_potc.calculate_main_potc_func)
                thread_reliability = threading.Thread(target=potc_class_reliability.calculate_main_potc_func)

                thread_potc.start()
                thread_reliability.start()

                thread_potc.join()
                thread_reliability.join()
            ...
            #set_value = potc_class.selected_route_count_dict
            #self.set_plain_text_edit_result_view_func(set_value)
...
class CalculatePOTC:
    ...
    def calculate_potc_and_reliability_func(self, process_count, mesafe_list, zaman_list, route_cnt, robot_cnt):
        ...
        reliability_value = self.system_main_dict["Robot Reliability"][robot_cnt - 1]
        potc_value = float(0)
